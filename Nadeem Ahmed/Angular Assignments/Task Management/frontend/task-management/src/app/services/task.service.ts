...
export class TaskService {
  ...
  createTask(data: TaskCreateRequest) {
    return this.http.post<TasksResponse>(`${environment.apiUrl}/tasks`, data);
  }
  ...
  updateTaskStatus(id: number, status: string) {
    return this.http.patch<TasksResponse>(`${environment.apiUrl}/tasks/${id}`, { status });
  }

  getAllTasks(): Observable<TasksResponse> {
    return this.http.get<TasksResponse>(`${environment.apiUrl}/tasks/all`);
  }
}
